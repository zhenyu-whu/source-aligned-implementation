...
6. 统一集成审核至少确认：
   ...
   - 每个已勾选任务都有 worker 提取的 design obligation checklist，且 checklist 中每个 obligation 都有实现证据和验证证据。
   - `Requirement Coverage` 三张表的每一行都有已勾选任务、实现证据、验证证据，并能回链到 Source Truth IDs、spec scenario 和 linked design obligation。
   - 涉及 runtime wiring 的默认导出/default dependency/default config 已被验证；不能只依赖 mock、stub、dependency injection、route intercept 或 fixture-only 测试。
   - 未引入明显跨章节冲突。
...
2. worker 只能勾选自己章节内已经完成且满足 `Proof`、linked spec scenario、linked design obligation、default runtime path verification 和 `Preserve` 约束的任务；未完成、未验证、trace 缺失、上下文不足、设计不清、default path 未证明或存在 blocker 的任务必须保持未勾选，并在最终回复中说明原因。
...
- 已完成的章节和任务。
- 每个 subagent 的关键改动范围。
- `Requirement Coverage` 审核结果，尤其是未覆盖或 proof 不足的 scenario / design obligation / default runtime path。
- 已执行的验证命令及结果。
- 未完成、被阻塞或需要用户决策的事项。
