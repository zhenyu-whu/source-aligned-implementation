...
| --- | --- | --- | --- | --- |
| <!-- ST-001; exactly one ID per row, no ranges such as ST-001-ST-010. Include every current-change Source Truth item and every ST ID referenced anywhere in this tasks file. --> | <!-- Short source-backed obligation summary, including deferred/non-goal boundary summaries when the row is a preserve or forbidden-drift item. --> | <!-- e.g. 2.1, 4.2 --> | <!-- e.g. 6.1, 6.3 --> | <!-- Source-equivalent proof: user interaction, API test, data assertion, worker/realtime path, security check, rendered layout check, forbidden-drift/static check, etc. --> |
